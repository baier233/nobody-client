...
Module::Module(HMODULE a_module)
{
	this->module = a_module;
	GetModuleInformation(GetCurrentProcess(), module, &moduleInfo, sizeof(MODULEINFO));
}

uint8_t* Module::pattern_scan(uint8_t pattern[], int size, int access) const
{
...
std::vector<uint8_t*> Module::pattern_scan_all(uint8_t pattern[], int size, int access) const
{
...
void* Module::getProcAddress(const char* name)
{
