...
namespace ImGui
{
	...
	IMGUI_API bool          Checkboxx(const char* label, ImTextureID image, bool* v);
	IMGUI_API bool          Button(const char* label, float x, float y, float width, float height);
	IMGUI_API bool          Checkboxx(const char* label, ImTextureID image, AbstractModule* module);
	IMGUI_API bool          Keybind(const char* label, ImTextureID image, int* key, int* mode, bool label_active = true);
