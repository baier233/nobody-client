...
struct tab_state
{
	ImVec4 background, background1, shadow, line, icon, text;
};
...
bool ImGui::Tab(const char* label, ImTextureID image, bool selectable, const ImVec2& size_arg)
{
	...
	it_anim->second.icon = ImLerp(it_anim->second.icon, selectable ? ImColor(0, 0, 0, 255) : ImColor(172, 187, 120, 255), g.IO.DeltaTime * 8.f);

	it_anim->second.background = ImLerp(it_anim->second.background, selectable ? ImColor(247, 248, 248, 255) : ImColor(247, 248, 248, 0), g.IO.DeltaTime * 8.f);

	it_anim->second.background1 = ImLerp(it_anim->second.background1, selectable ? ImColor(172, 187, 120, 255) : ImColor(172, 187, 120, 0), g.IO.DeltaTime * 8.f);

	GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(it_anim->second.background1), 10.f);

	ImGui::GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(15, 15), bb.Max + ImVec2(-15, -15), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));

	return pressed;
}
struct sub_tab_state
{
	ImVec4 background, background1, shadow, line, icon, text;
};
bool ImGui::SubTab(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	ImVec2 newSize = ImVec2(size.x, size.y + 10);
	const ImRect bb(pos, pos + size);
	const ImRect _RealBb({ pos.x - 10,pos.y - newSize.y }, pos + newSize);
	ItemSize(size, style.FramePadding.y);
	if (!ItemAdd(bb, id))
		return false;

	bool hovered, held;
	bool pressed = ButtonBehavior(_RealBb, id, &hovered, &held, flags);

	static std::map<ImGuiID, sub_tab_state> anim;
	auto it_anim = anim.find(id);

	if (it_anim == anim.end())
	{
		anim.insert({ id, sub_tab_state() });
		it_anim = anim.find(id);
	}

	it_anim->second.background = ImLerp(it_anim->second.background, selected ? ImColor(247, 248, 248, 255) : ImColor(247, 248, 248, 0), g.IO.DeltaTime * 8.f);

	it_anim->second.background1 = ImLerp(it_anim->second.background1, selected ? ImColor(172, 187, 120, 255) : ImColor(172, 187, 120, 0), g.IO.DeltaTime * 8.f);

	it_anim->second.shadow = ImLerp(it_anim->second.shadow, selected ? ImColor(172, 187, 120, 127) : ImColor(172, 187, 120, 0), g.IO.DeltaTime * 8.f);

	it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::active_sub : c::text_active, g.IO.DeltaTime * 6.f);

	//window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 5, bb.Min.y + 26), ImVec2(bb.Max.x - 5, bb.Max.y), ImColor(0, 106, 255, 255));
	GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(bb.Min.x + 23, bb.Min.y + 47), ImVec2(bb.Max.x - 7, bb.Max.y + 21), GetColorU32(it_anim->second.background), GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background));
	PushClipRect(ImVec2(bb.Min.x - 25, bb.Min.y - 20), ImVec2(bb.Max.x + size_arg.x + 50, bb.Max.y + size_arg.y + 25), true);
	GetWindowDrawList()->AddShadowCircle(ImVec2(bb.Min.x + 38, bb.Min.y + 47), 25.f, GetColorU32(it_anim->second.shadow), 120.f, ImVec2(0, 0));
	//GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Min.x + 35, bb.Min.y + 37), 15.f, GetColorU32(c::background_sun));
	PopClipRect();
	PushStyleColor(ImGuiCol_Text, ImGui::GetColorU32(it_anim->second.text));
	PushFont(Benzin_Medium_Font);
...
{
	ImVec4 Text_color, background, background1, rect, CheckMark;
	float rg;
	float mark_size;
};

bool ImGui::Checkboxx(const char* label, ImTextureID image, bool* v)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const float square_sz = 24.f, w = (GetContentRegionMax().x + 24 - g.Style.WindowPadding.x) - square_sz;
	const ImVec2 pos = window->DC.CursorPos;

	const ImRect total_bb(pos, pos + ImVec2(300, 29));
	ItemSize(total_bb, style.FramePadding.y);
	...
	it_anim->second.background = ImLerp(it_anim->second.background, *v ? ImColor(247, 248, 248, 255) : ImColor(19, 19, 19, 255), g.IO.DeltaTime * 8.f);

	it_anim->second.background1 = ImLerp(it_anim->second.background1, *v ? ImColor(172, 187, 120, 255) : ImColor(19, 19, 19, 255), g.IO.DeltaTime * 8.f);

	it_anim->second.CheckMark = ImLerp(it_anim->second.CheckMark, *v ? ImColor(0, 0, 0, 255) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 8.f);

	it_anim->second.Text_color = ImLerp(it_anim->second.Text_color, *v ? c::text_active : c::text_inactive, g.IO.DeltaTime * 8.f);

	it_anim->second.rect = ImLerp(it_anim->second.rect, *v ? (ImVec4)ImColor(0, 0, 0, 0) : c::rect, g.IO.DeltaTime * 8.f);

	it_anim->second.mark_size = ImLerp(it_anim->second.mark_size, *v ? 15.f : 0.f, g.IO.DeltaTime * 5);

	GetWindowDrawList()->AddRectFilledMultiColorRounded(ImVec2(check_bb.Min + ImVec2(w - 30, -5)), check_bb.Min + ImVec2(w, 25), GetColorU32(ImGuiCol_PopupBg), GetColorU32(it_anim->second.background), GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background), 5.f, ImDrawFlags_RoundCornersAll);
	// GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(w - 15, 1), check_bb.Max + ImVec2(w, -1), GetColorU32(it_anim->second.background), 100.f);

	GetWindowDrawList()->AddRect(ImVec2(check_bb.Min + ImVec2(w - 31, -6)), check_bb.Min + ImVec2(w + 1, 26), GetColorU32(it_anim->second.rect), 5.f);

	// RenderCheckMark(window->DrawList, total_bb.Min + ImVec2(w - 16, 10 + it_anim->second.rg) - ImVec2(it_anim->second.mark_size / 2, it_anim->second.mark_size / 2), GetColorU32(it_anim->second.CheckMark), 10);

	ImGui::GetWindowDrawList()->AddImage(image, total_bb.Min + ImVec2(w - 25, 0), total_bb.Min + ImVec2(w - 5, 20), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.CheckMark));

	GetWindowDrawList()->AddText(ImVec2(check_bb.Min.x, check_bb.Min.y), GetColorU32(it_anim->second.Text_color), label);

	IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
	return pressed;
}

IMGUI_API bool ImGui::Button(const char* label, float offsetX, float offsetY, float buttonWidth, float buttonHeight)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImRect total_bb(pos + ImVec2(offsetX, offsetY), pos + ImVec2(offsetX + buttonWidth, offsetY + buttonHeight));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, id))
	{
		IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
	...
	ImColor bg = ImColor(11, 12, 13, 255);
	ImColor color1 = ImColor(236, 239, 229, 255);
	ImColor color2 = ImColor(183, 196, 139, 255);
	ImColor text_color = ImColor(255, 255, 255, 255);
	ImColor text_color2 = ImColor(0, 0, 0, 255);
	ImColor shadow_color = ImColor(0, 0, 0, 128); // 阴影颜色

	window->DrawList->AddRectFilledMultiColorRounded(total_bb.Min, total_bb.Max, GetColorU32(ImGuiCol_PopupBg), color1, color2, color2, color1, 5.f, ImDrawFlags_RoundCornersAll);
	window->DrawList->AddText(ImVec2(total_bb.Min.x + (buttonWidth - label_size.x) * 0.5f + 2, total_bb.Min.y + (buttonHeight - label_size.y) * 0.5f + 2), text_color2, label); // 绘制阴影文本，位置偏移更大
	//window->DrawList->AddText(ImVec2(total_bb.Min.x + (buttonWidth - label_size.x) * 0.5f, total_bb.Min.y + (buttonHeight - label_size.y) * 0.5f), text_color2, label); // 绘制正常文本

	return pressed;
}


#include "../../src/base/moduleManager/ModuleManager.h"
bool ImGui::Checkboxx(const char* label, ImTextureID image, AbstractModule* module)
{
	auto value = module->getToggle();
	auto v = &value;
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::Keybind(const char* label, ImTextureID image, int* key, int* mode, bool label_active)
{
	...
	const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(width, 35.f));
	...
	it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? c::text_active : c::text_inactive, ImGui::GetIO().DeltaTime * 6.f);
	it_anim->second.slow = ImLerp(it_anim->second.slow, clickable.Min.x - rect.Min.x, g.IO.DeltaTime * 15.f);

	window->DrawList->AddRectFilled(ImVec2(it_anim->second.slow + rect.Min.x, clickable.Min.y - 6), clickable.Max - ImVec2(0, 7), GetColorU32(it_anim->second.background), 5.f, ImDrawFlags_RoundCornersRight);
	window->DrawList->AddRect(ImVec2(it_anim->second.slow + rect.Min.x, clickable.Min.y - 6), clickable.Max - ImVec2(0, 7), GetColorU32(c::rect), 5.f, ImDrawFlags_RoundCornersRight);
	window->DrawList->AddRect(ImVec2(it_anim->second.slow + rect.Min.x - 31, clickable.Min.y - 6), clickable.Max - ImVec2(0, 7), GetColorU32(c::rect), 5.f);
	window->DrawList->AddRectFilled(ImVec2(it_anim->second.slow + rect.Min.x - 30, clickable.Min.y - 5), ImVec2(it_anim->second.slow + rect.Min.x + 2, clickable.Max.y - 8), GetColorU32(c::background), 5.f, ImDrawFlags_RoundCornersLeft);
	ImGui::GetWindowDrawList()->AddImage(image, ImVec2(it_anim->second.slow + rect.Min.x - 25, clickable.Min.y - 0), ImVec2(it_anim->second.slow + rect.Min.x - 5, clickable.Max.y - 13), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::text_active));
	...
	PushStyleColor(ImGuiCol_Text, c::text_inactive);
	RenderTextClipped(clickable.Min + ImVec2(0, -13), clickable.Max, buf_display, NULL, &label_size, ImVec2(0.5f, 0.5f));
	PopStyleColor();
	ImGui::PopClipRect();
	...
	PushStyleColor(ImGuiCol_WindowBg, c::background);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
	...
	const ImRect bb(window->DC.CursorPos + ImVec2(0, 27), window->DC.CursorPos + ImVec2(w - 1, 61));
	...
	it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? c::text_active : c::text_inactive, g.IO.DeltaTime * 6.f);
	it_anim->second.icon = ImLerp(it_anim->second.icon, it_anim->second.opened_combo ? c::text_active : c::text_active, g.IO.DeltaTime * 6.f);
	...
	window->DrawList->AddRect(ImVec2(bb.Min.x - 1, bb.Min.y - 3), ImVec2(bb.Max.x + 1, bb.Max.y), GetColorU32(c::rect), 5.f);
	GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y - 2), ImVec2(bb.Max.x, bb.Max.y - 1), GetColorU32(c::input_bg), 5.f);
   ...
   // PushFont(sf_pro_text_1);
	...
	GetWindowDrawList()->AddImage(image::arrows, ImVec2(value_x2, bb.Min.y) - ImVec2(5, 0), bb.Max - ImVec2(7, 5), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));
	...
	window->DrawList->AddText(ImVec2(bb.Min.x, bb.Min.y - 32), GetColorU32(it_anim->second.text), label);
	// PopFont();
	...
	PushStyleColor(ImGuiCol_WindowBg, c::input_bg);
	PushStyleVar(ImGuiStyleVar_WindowRounding, 5.f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
	ItemSize(ImRect(total_bb.Min, total_bb.Max + ImVec2(0, 35.f)));
	if (!ItemAdd(total_bb, id, &frame_bb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
		return false;
	...
	it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::text_active : c::text_inactive, g.IO.DeltaTime * 6.f);

	it_anim->second.circle = ImLerp(it_anim->second.circle, IsItemActive() ? ImColor(255, 255, 255, 255) : ImColor(255, 255, 255, 255), g.IO.DeltaTime * 8.f);

	it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ImColor(247, 248, 248, 255) : ImColor(247, 248, 248, 255), g.IO.DeltaTime * 8.f);

	it_anim->second.background1 = ImLerp(it_anim->second.background1, IsItemActive() ? ImColor(172, 187, 120, 255) : ImColor(172, 187, 120, 255), g.IO.DeltaTime * 8.f);

	it_anim->second.background_rect = ImLerp(it_anim->second.background_rect, IsItemActive() ? ImColor(33, 33, 33, 255) : ImColor(33, 33, 33, 255), g.IO.DeltaTime * 8.f);

	it_anim->second.circle_smile = ImLerp(it_anim->second.circle_smile, IsItemActive() ? ImColor(181, 176, 176, 255) : ImColor(181, 176, 176, 255), g.IO.DeltaTime * 8.f);
	...
	GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 13), frame_bb.Max + ImVec2(0, 4), GetColorU32(it_anim->second.background_rect), 100.f);

	if (grab_bb.Max.x > grab_bb.Min.x) {

		it_anim->second.slow_anim = ImLerp(it_anim->second.slow_anim, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 25.f);

		GetWindowDrawList()->AddShadowCircle(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 5, grab_bb.Max.y + 2), 10.f, ImColor(172, 187, 120, 255), 28.f, ImVec2(0, 0));

		GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 5, grab_bb.Max.y + 2), 8.f, GetColorU32(it_anim->second.circle), 100.f);

		GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 5, grab_bb.Max.y + 2), 4.f, GetColorU32(it_anim->second.circle_smile), 100.f);

		GetWindowDrawList()->AddRectFilledMultiColorRounded(frame_bb.Min + ImVec2(0, 13), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y + 6), GetColorU32(ImGuiCol_PopupBg), GetColorU32(it_anim->second.background), GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background), 100.f, ImDrawFlags_RoundCornersAll);
	}

	PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
	RenderTextClipped(frame_bb.Min - ImVec2(0, 27), frame_bb.Max - ImVec2(0, 19), value_buf, value_buf_end, NULL, ImVec2(1.0f, 1.0f));
	PopStyleColor();

	GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w, frame_bb.Min.y - 21), GetColorU32(it_anim->second.text), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	ImGui::SetCursorPos(GetCursorPos() + ImVec2(1, 0));
	...
	const ImGuiID id = window->GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() + 70, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 4.0f); // Arbitrary default of 8 lines high for multi-line
	const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y + 32);

	const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 25), window->DC.CursorPos + frame_size + ImVec2(0, 29));
	const ImRect total_bb(frame_bb.Min, frame_bb.Min + total_size);
	const ImRect frame_bb1(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(280, 70));

	const ImVec2 pos = window->DC.CursorPos;
	const ImRect total_bb1(pos, pos + ImVec2(280, 45));
	...
	it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? c::text_active : c::text_inactive, g.IO.DeltaTime * 10.f);
	...
	if (!is_multiline)
	{
		...
		RenderFrame(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), GetColorU32(c::input_bg), false, 5.f);
		window->DrawList->AddRect(ImVec2(frame_bb.Min.x - 2, frame_bb.Min.y - 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), GetColorU32(c::rect), 5.f);
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Min.x + 40, frame_bb.Max.y), GetColorU32(c::background_1), 5.f, ImDrawFlags_RoundCornersLeft);
		ImGui::GetWindowDrawList()->AddImage(image::input, ImVec2(frame_bb.Min.x + 10, frame_bb.Min.y + 8), ImVec2(frame_bb.Min.x + 30, frame_bb.Max.y - 7), ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, 255));
	}
	...
	if (render_cursor || render_selection)
	{
		...
		if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
		{
			...
			draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(45, 5), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
			//  ImGui::PopFont();
		}
		...
		if (render_cursor)
		{
			...
			if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {
				...
				draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.value + 44, 9), frame_bb.Min + ImVec2(it_anim->second.value + 45, 26), GetColorU32(c::cursor));
			}
	...
	else
	{
		...
		if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
		{
			...
			ImU32 col = GetColorU32(it_anim->second.text_color);
			draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos + ImVec2(45, 5), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
	...
	window->DrawList->AddText(ImVec2(frame_bb.Max.x - CalcItemWidth() - 71, frame_bb.Min.y - 32), GetColorU32(it_anim->second.text_color), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
	{
		...
		PushStyleColor(ImGuiCol_PopupBg, GetColorU32(c::background));
		PushStyleColor(ImGuiCol_Border, GetColorU32(c::background));
	...
	if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
	{
		...
		ImGui::GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, 0), GetColorU32(c::text_active), label);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
	...
	const ImRect bb(window->DC.CursorPos + ImVec2(w - 8, -5), window->DC.CursorPos + size + ImVec2(w + 1, 3));
	...
	else
	{
		...
		if (col_source.w < 1.0f)
			...
			window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 5.f);
	}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::text_active : c::text_inactive, g.IO.DeltaTime * 6.f);

	PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
	RenderTextClipped(text_min + ImVec2(5, 5), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
	PopStyleColor();

	window->DrawList->AddCircleFilled(ImVec2(bb.Max.x - 25, bb.Max.y - 10), 4.f, GetColorU32(it_anim->second.text));
